            ...
            <div class="card" style="border-radius: 20px;">
                <div class="card-header d-flex justify-content-between align-items-center flex-wrap">
                    <h4 class="card-title mb-0 text-dark">
                        <i class="now-ui-icons"></i> {{ __('Edit Data Sales') }}
                    </h4>
                </div>
                    ...
                    <form action="{{ route('sales.update', $sales->id) }}" method="POST">
                        ...
                        <div class="form-group">
                            ...
                            <div class="btn-group btn-group-toggle d-flex justify-content-start" data-toggle="buttons">
                                <label
                                    class="btn btn-round btn-outline-success {{ strtolower($sales->status) == 'aktif' ? 'active' : '' }} mr-2">
                                    <input type="radio" name="status" id="status-aktif" value="Aktif"
                                        {{ strtolower($sales->status) == 'aktif' ? 'checked' : '' }} autocomplete="off">
                                    <i class="now-ui-icons ui-1_check mr-1"></i> Aktif
                                </label>

                                <label
                                    class="btn btn-round btn-outline-danger {{ strtolower($sales->status) == 'nonaktif' ? 'active' : '' }}">
                                    <input type="radio" name="status" id="status-nonaktif" value="Nonaktif"
                                        {{ strtolower($sales->status) == 'nonaktif' ? 'checked' : '' }}
                                        autocomplete="off">
